...
export const uniq_line = (data: string) => uniq(data.split('\n').map(x => x.trim())).join('\n');
...
export const sort_asc = (data: string) => data.split('\n').map(x => x.trim()).sort().join('\n');
export const sort_desc = (data: string) => data.split('\n').map(x => x.trim()).sort().reverse().join('\n');
export const sort_len_asc = (data: string) => data.split('\n').map(x => x.trim()).sort((a, b) => a.length - b.length).join('\n');
export const sort_len_desc = (data: string) => data.split('\n').map(x => x.trim()).sort((a, b) => b.length - a.length).join('\n');
